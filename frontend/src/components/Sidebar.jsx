        ...
        <aside className="fixed h-screen flex flex-col">
            <nav className="max:h-full m-2 bg-sky-500 shadow-lg rounded flex-col">
                <div className={extended ?
                 "nav-top-container ":
                 "nav-top-container flex-col"} >
                    <div className="flex">
                        <img src={logo} className="size-16 inline-block mb-2"></img>
                        {extended ? (<p className="self-end mb-4 ml-2 text-2xl text-white inline-block font-bold">Blume News</p>) : undefined}
                    </div>
                    <button 
                ...
                <ul className={extended ? "flex flex-col items-start" : 
                "flex flex-col items-center"}>
                    <li> 
                        <button className={extended ? 
                "flex bg-sky-400 ml-4 px-2 rounded shadow-lg mb-2 hover:bg-sky-300 items-center text-left" : undefined}
                ...
                "flex p-3 items-center flex-col"}>
                    ...
                    <div className={ extended ? 
                    "flex justify-between ml-3" : "flex"}>
                        { extended && (
                        <div className="leading-4 mr-4">
                        <h4 className="font-semibold">{sessionStorage.getItem("username")}</h4>
                        <span className="text-xs text-gray-600">{sessionStorage.getItem("email")}</span>
                        </div>)}
                        <button className="nav-button bx bx-log-out bx-sm mb-1" onClick={() => {handleNavigation("logout")}}></button>    
                    </div>
                </div>
            </nav>
        </aside>
